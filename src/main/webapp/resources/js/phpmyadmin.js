...
function suggestPassword(a) {
    ...
    b.value = "";
    for (i = 0; i < 16; i++)
        b.value += "abcdefhjmnpqrstuvwxyz23456789ABCDEFGHJKLMNPQRSTUVWYXZ".charAt(Math.floor(Math.random() * 53));
...
function parseVersionString(a) {
    ...
    var b = 0, c = a.split("-");
    if (c.length >= 2)
        if (c[1].substr(0, 2) == "rc")
            b = -20 - parseInt(c[1].substr(2));
        else if (c[1].substr(0, 4) == "beta")
            b = -40 - parseInt(c[1].substr(4));
        else if (c[1].substr(0, 5) == "alpha")
            b = -60 - parseInt(c[1].substr(5));
        else if (c[1].substr(0, 3) == "dev")
            b = 0;
    var d = a.split(".");
    a = parseInt(d[0]) || 0;
    c = parseInt(d[1]) || 0;
...
function PMA_addDatepicker(a) {
    var b = false;
    if (a.is(".datetimefield"))
        b = true;
    a.datepicker({showOn: "button", buttonImage: themeCalendarImage, buttonImageOnly: true, duration: "", time24h: true, stepMinutes: 1, stepHours: 1, showTime: b, dateFormat: "yy-mm-dd", altTimeField: "", beforeShow: function() {
...
function confirmQuery(a, b) {
    if (PMA_messages.strDoYouReally == "")
        return true;
    else if (typeof b.value.replace == "undefined")
        return true;
    else {
        if (PMA_messages.strNoDropDatabases != "")
            if (/(^|;)\s*DROP\s+(IF EXISTS\s+)?DATABASE\s/i.test(b.value)) {
...
function checkSqlQuery(a) {
    ...
    if (typeof b.value.replace == "undefined") {
        if ((c = b.value == "" ? 1 : 0) && typeof a.elements.sql_file != "undefined")
            c = a.elements.sql_file.value == "" ? 1 : 0;
        if (c && typeof a.elements.sql_localfile != "undefined")
            c = a.elements.sql_localfile.value == "" ? 1 : 0;
        if (c && typeof a.elements.id_bookmark != "undefined")
            c = a.elements.id_bookmark.value == null || a.elements.id_bookmark.value == ""
    } else {
        var d = /\s+/;
        if (typeof a.elements.sql_file != "undefined" && a.elements.sql_file.value.replace(d,
                "") != "")
            return true;
        if (typeof a.elements.sql_localfile != "undefined" && a.elements.sql_localfile.value.replace(d, "") != "")
            return true;
        if (c && typeof a.elements.id_bookmark != "undefined" && (a.elements.id_bookmark.value != null || a.elements.id_bookmark.value != "") && a.elements.id_bookmark.selectedIndex != 0)
            return true;
        if (b.value.replace(d, "") != "")
            return confirmQuery(a, b) ? true : false;
        a.reset();
        c = 1
    }
    if (c) {
...
function checkTransmitDump(a, b) {
    ...
    if (b == "zip" && c.zip.checked) {
        if (!c.asfile.checked)
            a.elements.asfile.checked = true;
        if (typeof c.gzip != "undefined" && c.gzip.checked)
            a.elements.gzip.checked = false;
        if (typeof c.bzip != "undefined" && c.bzip.checked)
            a.elements.bzip.checked = false
    } else if (b == "gzip" && c.gzip.checked) {
        if (!c.asfile.checked)
            a.elements.asfile.checked = true;
        if (typeof c.zip != "undefined" && c.zip.checked)
            a.elements.zip.checked = false;
        if (typeof c.bzip != "undefined" && c.bzip.checked)
            a.elements.bzip.checked =
                    false
...
function insertQuery(a) {
    ...
    if (b.options.length > 0) {
        ...
        if (a == "selectall")
            c = "SELECT * FROM `" + d + "` WHERE 1";
        else if (a == "select")
            c = "SELECT " + e + " FROM `" + d + "` WHERE 1";
        else if (a == "insert")
            c = "INSERT INTO `" + d + "`(" + e + ") VALUES (" + f + ")";
        else if (a == "update")
            c = "UPDATE `" +
                    d + "` SET " + j + " WHERE 1";
        else if (a == "delete")
            c = "DELETE FROM `" + d + "` WHERE 1";
...
function getElement(a, b) {
    if (document.layers) {
        b = b ? b : self;
        if (b.document.layers[a])
            return b.document.layers[a];
        for (W = 0; W < b.document.layers.length; )
            return getElement(a, b.document.layers[W])
    }
    if (document.all)
...
$(document).ready(function() {
    ...
    $("#sqlquery").focus();
    if ($("#input_username"))
        $("#input_username").val() == "" ? $("#input_username").focus() : $("#input_password").focus()
});
function PMA_ajaxShowMessage(a, b) {
    ...
    if (ajax_message_init)
        $("#loading").stop(true, true).html(c).fadeIn("medium").delay(d).fadeOut("medium", function() {
            $(this).html("").hide()
        });
    else {
        $(function() {
...
$(document).ready(function() {
    $("#create_table_form_minimal.ajax").live("submit", function(a) {
        ...
        $.get($form.attr("action"), $form.serialize(), function(e) {
            e.success != undefined && e.success == false ? $('<div id="create_table_dialog"></div>').append(e.error).dialog({title: PMA_messages.strCreateTable,
                height: 230, width: 900, open: PMA_verifyTypeOfAllColumns, buttons: c}).find("fieldset").remove() : $('<div id="create_table_dialog"></div>').append(e).dialog({title: PMA_messages.strCreateTable, height: 600, width: 900, open: PMA_verifyTypeOfAllColumns, buttons: b});
    ...
    $("#create_table_form input[name=do_save_data]").live("click", function(a) {
        a.preventDefault();
        a = $("#create_table_form");
        if (checkTableEditForm(a[0], a.find("input[name=orig_num_fields]").val()))
            if (a.hasClass("ajax")) {
                ...
                $.post(a.attr("action"), a.serialize() + "&do_save_data=" + $(this).val(), function(b) {
                    if (b.success == true) {
                        ...
                        if (c.length == 0)
                            window.parent && window.parent.frame_content && window.parent.frame_content.location.reload();
                        else {
...
$(document).ready(function() {
    ...
    $("#change_password_anchor.ajax").live("click", function(a) {
        ...
        $.get($(this).attr("href"), {ajax_request: true}, function(c) {
            $('<div id="change_password_dialog"></div>').dialog({title: PMA_messages.strChangePassword, width: 600,
                close: function() {
                    $(this).remove()
...
$(document).ready(function() {
    $("a[class='open_enum_editor']").live("click", function() {
        ...
        d = d / 2 - f / 2;
        c = c / 2 - e / 2;
        $("#enum_editor").css({position: "absolute", top: d, left: c, width: e, height: f});
        ...
        for (h = 0; h < f.length; h++)
            $("#enum_editor #values").append("<input type='text' value='" + f[h] + "' />");
    ...
    if ($("input[type='hidden'][name='table_type']").val() == "table") {
        ...
        $(".structure_actions_dropdown").each(function() {
            ...
            c.offset({top: e, left: d})
        });
        var b = $("select[name='after_field']");
        $("iframe[class='IE_hack']").width(b.width()).height(b.height()).offset({top: b.offset().top, left: b.offset().left});
        a.find("td[class='more_opts']").mouseenter(function() {
            $.browser.msie && $.browser.version == "6.0" && $("iframe[class='IE_hack']").show().width(b.width() + 4).height(b.height() + 4).offset({top: b.offset().top, left: b.offset().left});
            ...
            if ($.browser.msie) {
                ...
                $(this).children(".structure_actions_dropdown").offset({top: d, left: c})
            }
        }).mouseleave(function() {
...
$(document).ready(function() {
    ...
    $("img[class='footnotemarker']").each(function() {
        ...
        $(this).qtip({content: a, show: {delay: 0}, hide: {delay: 1E3}, style: {background: "#ffffcc"}})
    })
});
function menuResize() {
    ...
    if (h > 0) {
        for (k = h; k < a.length - 1; k++)
            $(a[k])[j ? "prependTo" : "appendTo"](e);
        c.addClass("shown")
    } else if (j) {
        g -= d;
...
$(function() {
    var a = $("#topmenu");
    if (a.length != 0) {
        var b = $("<a />", {href: "#", "class": "tab"}).text(PMA_messages.strMore).click(function(d) {
            d.preventDefault()
